...
public class YuGongController extends AbstractYuGongLifeCycle {
    ...
    public void start() {
        ...
        log.info("## prepare start tables[{}] with concurrent[{}]", instances.size(), threadSize);
        ...
        schedule.scheduleWithFixedDelay(() -> {
            try {
                ...
                log.error("print progress failed", e);
            }
...
//            canalServerPort);
    ...
    private Collection<TableHolder> initTables() {
        log.info("check source tables read privileges ...");
        ...
        // }
        log.info("check source tables is ok.");
        return tables;
    }
    ...
    private DataTranslator buildExtKeys(Table table, String tableStr, DbType targetDbType) {
        ...
        if (targetDbType.isDRDS()) {
            ...
            if (extKey != null && !StringUtils.equalsIgnoreCase(drdsExtKey, extKey)) {
                log.warn("table:[{}] is not matched drds shardKey:[{}]", tableStr, drdsExtKey);
            }
    ...
    @EqualsAndHashCode(exclude = {"ignoreSchema", "translators"})
    private static class TableHolder {

        Table table;

        boolean ignoreSchema;
        List<DataTranslator> translators;

        public TableHolder(Table table) {
            this.table = table;
            this.ignoreSchema = false;
            this.translators = Lists.newArrayList();
        }
    ...
    private void preCheckMlogGrant(DataSource ds) {
        ...
        log.info("check mlog privileges ...");
        ...
        log.info("check mlog privileges is ok");
    }
}
